...
function fluctuateRates() {
    const noiseEUR = (Math.random() - 0.5) * 0.1; // Сильный шум
    const noiseCNY = (Math.random() - 0.5) * 1.0;

    rates.EUR += noiseEUR;
    rates.CNY += noiseCNY;

    rates.EUR = Math.max(0.5, Math.min(10.0, rates.EUR));
    rates.CNY = Math.max(5.0, Math.min(100.0, rates.CNY));
